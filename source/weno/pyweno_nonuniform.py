...
def jiang_shu_smoothness_coefficients(k, x):
    ...
    filename = os.path.join(filename, 'jiang_shu' + str(k) + '.pkl')
    try:
        ...
        for r in range(0, k):
            ...
            args.append(X1)
            args.append(X2)
            l = sympy.utilities.lambdify(args, ppp[r])
            fncs.append(l)
    ...
    for i in range(k-1, N-k+1):
        for r in range(0, k):
            ...
            # substitutions = [(xs[j], X[i-r+j]) for j in range(k + 1)]
            # substitutions.append((X1, X[i]))
            # substitutions.append((X2, X[i+1]))
            args = []
            for j in range(0, k + 1):
                args.append(X[i - r + j])
            args.append(X[i])
            args.append(X[i + 1])
            # polynomial = polynomial.subs(substitutions)
            polynomial = fncs[r](*args)
            # pick out coefficients
            for m in range(k):
                for n in range(m, k):
                    c = polynomial.coeff(ys[m]*ys[n])
                    if c is not None:

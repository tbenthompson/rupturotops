...
class Waves(Experiment):
    ...
    TODO:
        --ABCs at bottom and sides, free surface at top
        --Frontend the assembly
        --Figure out how to do explicit solving
        --Implement Fault BCs
    """
    def _initialize(self):
        ...
        self.SS = dfn.VectorFunctionSpace(self.mesh, 'CG', 2) #stress fnc space
        self.MS = self.VS * self.SS

        class InitCond(dfn.Expression):
            def eval(self, value, x):
                value[0] = np.exp(- (x[0] * x[0] + x[1] * x[1]) / 5000000.0)
                value[1] = 0.0
                value[2] = 0.0
        ...
        self.bc = dfn.DirichletBC(self.VS, dfn.Constant('0.0'), lambda x, on_bndry: on_bndry)
        ...
        self.solver = LinearProblem(dfn.lhs(self.form), dfn.rhs(self.form), [self.bc], dfn.LUSolver())
        # J =
    ...
    def _compute(self):
        ...
        vtk_file = dfn.File("elasticity.pvd")
